...
# Ensure tests import local src package, not installed site-packages
...
class TestProcessAllConfig:
    ...
    def test_validate_papers_directory_checks_pdfs(self, tmp_path):
        ...
        assert process_all.validate_papers_directory(
            str(papers_dir),
            [paper_folder],
            merge_only=False
        ) is False
    ...
    def test_validate_papers_directory_with_pdfs(self, tmp_path):
        ...
        # Should not raise
        process_all.validate_papers_directory(
            str(papers_dir),
            [paper_folder],
            merge_only=False
        )
...
class TestVAProcessPapersConfig:
    ...
    def test_process_papers_uses_env_api_key(
        ...
        va_process_papers.process_papers(
    ...
    @patch("metabeeai.process_pdfs.va_process_papers.requests.post")
    def test_process_papers_uses_config_file_api_key(
        self, mock_post, monkeypatch, tmp_path
    ):
    ...
    @patch("metabeeai.process_pdfs.va_process_papers.requests.post")
    def test_process_papers_config_overrides_env(
        self, mock_post, monkeypatch, tmp_path
    ):
